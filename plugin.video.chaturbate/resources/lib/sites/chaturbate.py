...
site = AdultSite(
    'chaturbate',
    '{0} {1}'.format(mt.c(mt.CYAN, 'Chaturbate'), mt.c(mt.PINK, 'TV')),
    bu,
    'miami-brand.png',
    'chaturbate',
    True,
...
GENDER_CFG = {
    'f': ('chatfemale', 'gender_female', 'female', mt.PINK),
    'm': ('chatmale', 'gender_male', 'male', mt.CYAN),
    'c': ('chatcouple', 'gender_couple', 'couple', mt.PINK),
    't': ('chattrans', 'gender_trans', 'trans', mt.VIOLET),
}

_GENDER_SUBFILTERS = (
    ({'new_cams': 'true'}, 'sub_new_cams'),
    ({'from_age': 18, 'to_age': 19}, 'sub_teen'),
    ({'from_age': 20, 'to_age': 30}, 'sub_age_20_30'),
    ({'from_age': 30, 'to_age': 50}, 'sub_age_30_50'),
    ({'from_age': 50, 'to_age': 200}, 'sub_mature'),
    ({'regions': 'NA'}, 'sub_na'),
    ({'regions': 'SA'}, 'sub_sa'),
    ({'regions': 'ER'}, 'sub_eu'),
    ({'regions': 'AS'}, 'sub_asia'),
...
def Main():
    i = utils.i18n
    site.add_dir(mt.c(mt.ORANGE, i('menu_refresh')), '', 'clean_database', mt.img('refresh'), Folder=False)
    site.add_dir(mt.c(mt.CYAN, i('menu_search')), site.url + 'ax/search/?keywords=', 'Search', mt.img('search'))

    site.add_dir(mt.c(mt.PINK, i('menu_featured')), rapi + '?limit=100&offset=0', 'List', mt.img('featured'))
    site.add_dir(mt.c(mt.ORANGE, i('menu_top_cams')), bu + 'api/ts/contest/leaderboard/', 'topCams', mt.img('top'))
    site.add_dir(mt.c(mt.FAV, i('menu_online_fav')), bu, 'onlineFav', mt.img('favorites'))
    site.add_dir(mt.c(mt.VIOLET, i('menu_followed')), rapi + '?enable_recommendations=true&follow=true&limit=100&offline=false&offset=0', 'List', mt.img('follow'))
    site.add_dir(mt.c(mt.FAV, i('menu_saved_fav')), '1', 'favorites.List', mt.img('saved'))

    site.add_dir(mt.c(mt.VIOLET, i('setup_menu')), '', 'SetupMenu', mt.img('deps'))

    for code, (setting, label_key, img_key, color) in GENDER_CFG.items():
        if addon.getSetting(setting) == 'true':
            site.add_dir(mt.c(color, i(label_key)), '', 'GenderMenu', mt.img(img_key), section=code)
...
def SetupMenu(url=''):
    ...
    site.add_dir(
        mt.c(mt.CYAN, i('install_deps')),
        '', 'utils.install_missing_deps', mt.img('deps'), Folder=False,
    )
    site.add_dir(
        mt.c(mt.FAV, i('migrate_cumination')),
        '', 'favorites.migrate_from_cumination', mt.img('saved'), Folder=False,
    )
    utils.eod()
...
def GenderMenu(url='', section='f'):
    ...
    if not cfg:
        ...
        utils.eod()
        return
    _, label_key, img_key, color = cfg
    label = utils.i18n(label_key)
    site.add_dir(mt.c(color, utils.i18n('menu_browse_all').format(label)), rapi + '?genders={0}&limit=100&offset=0'.format(section), 'List', mt.img(img_key))
    site.add_dir(mt.c(mt.CYAN, utils.i18n('menu_tags').format(label)), tapi + '?sort=ht&page=1&g={0}&limit=100'.format(section), 'Tags', mt.img('tags'))
    _gender_subdirs(section)
    utils.eod()


def _sub_list(gender, label_key, **params):
    label = utils.i18n(label_key)
    parts = ['genders={0}'.format(gender), 'limit=100', 'offset=0']
    parts.extend('{0}={1}'.format(k, v) for k, v in sorted(params.items()))
    site.add_dir(mt.c(mt.DIM, label), rapi + '?' + '&'.join(parts), 'List', mt.img('list'))


def _gender_subdirs(gender):
    for params, label_key in _GENDER_SUBFILTERS:
        _sub_list(gender, label_key, **params)
...
def List(url, page=1):
    ...
    if 'follow=true' in url and 'offline=false' in url:
        site.add_dir(mt.c(mt.VIOLET, utils.i18n('menu_offline_rooms')), rapi + '?enable_recommendations=true&follow=true&limit=100&offline=true&offset=0', 'List', mt.img('list'))
    ...
    for model in models:
        ...
        if 'offline=true' not in url:
            subject = utils.cleantext(subject) + "[CR][CR]" + mt.c(mt.PINK, 'Location: ') + location + "[CR]" \
                + mt.c(mt.PINK, 'Duration: ') + "{0}[CR]".format(Online(model.get('start_timestamp'))) \
                + mt.c(mt.PINK, 'Watching: ') + "{0}[CR]".format(model.get('num_users')) \
                + mt.c(mt.PINK, 'Followers: ') + "{0}".format(model.get('num_followers'))
            name = '{0} {1} {2}'.format(name, mt.c(mt.PINK, '[{0}]'.format(age)), model.get('current_show'))
        else:
            subject = utils.cleantext(subject)
            if model.get('start_timestamp'):
                ago = re.sub(r'^\s*(\d+\s+\S+).*$', r'\1', Online(model.get('start_timestamp')))
                name = '{0} {1} {2}'.format(name, mt.c(mt.PINK, '[{0}]'.format(age)), mt.c(mt.CYAN, '{0} ago'.format(ago)))
            else:
                name = '{0} {1}'.format(name, mt.c(mt.PINK, '[{0}]'.format(age)))
        tags = model.get('tags')
        if tags:
            tags = '[COLOR hotpink]#[/COLOR]' + ', [COLOR hotpink]#[/COLOR]'.join(tags)
        ...
        contextmenu = [(mt.c(mt.CYAN, 'Follow ') + name, 'RunPlugin(' + contextfollow + ')')] if not follow else [(mt.c(mt.CYAN, 'Unfollow ') + name, 'RunPlugin(' + contextunfollow + ')')]
        contextrecord = (utils.addon_sys + "?mode=chaturbate.Record&id=" + urllib_parse.quote_plus(id))
        contextmenu.append(('[COLOR cyan]Find recordings featuring [/COLOR]{}[COLOR cyan] on Cloudbate[/COLOR]'.format(id), 'RunPlugin(' + contextrecord + ')'))
    ...
    if nextp:
        ...
        site.add_dir(utils.i18n('menu_next_page').format(page - 1, lastpg), nurl, 'List', site.img_next, page)
...
def SList(url):
    ...
    for model in jlist.get('online', []):
        ...
        contextmenu = [('[COLOR cyan]Follow [/COLOR]{}'.format(model), 'RunPlugin(' + contextfollow + ')'), ('[COLOR cyan]Unfollow [/COLOR]{}'.format(model), 'RunPlugin(' + contextunfollow + ')')]
...
def clean_database(showdialog=True):
    ...
    try:
        with conn:
            ...
            if showdialog:
                utils.notify('ChaturbateTV', utils.i18n('thumb_cleared'))
    except:
        pass
...
def Search(url, keyword=None):
    if not keyword:
        keyword = utils._get_keyboard(heading=utils.i18n('srch_for'))
        if not keyword:
            utils.eod()
            return
    url += urllib_parse.quote_plus(keyword)
    SList(url)
...
def topCams(url):
    ...
    for iTop in jsonTop:
        subject = '[COLOR hotpink]Name: [/COLOR]' + iTop['room_user'] + '[CR]' \
            + '[CR][COLOR hotpink]Points: [/COLOR]' + str(iTop['points']) + '[CR]' \
            + '[COLOR hotpink]Watching: [/COLOR]' + str(iTop['viewers'])
...
def Tags(url, page=1):
    ...
    if page * 100 < total:
        ...
        site.add_dir(utils.i18n('menu_next_page').format(page, lastpg), np_url, 'Tags', site.img_next, page=page + 1)
    utils.eod()
        ...
        if stripped_name in model_lookup:
            ...
            if "current_show" in model:
                if model["current_show"] != "public":
                    current_show = '[COLOR cyan] {}[/COLOR]'.format(model["current_show"])
            ...
            subject = utils.cleantext(subject.split(' #')[0]) + "[CR][CR][COLOR hotpink]Location: [/COLOR]" + location + "[CR]" \
                + "[COLOR hotpink]Duration: [/COLOR]" + str(round(model["seconds_online"] / 3600, 1)) + " hrs[CR]" \
                + "[COLOR hotpink]Watching: [/COLOR]" + str(model["num_users"]) + " viewers"
            tags = '[COLOR hotpink]#[/COLOR]' + ', [COLOR hotpink]#[/COLOR]'.join(model["tags"])
...
def login():
    ...
    if '<h1>Chaturbate Login</h1>' in response:
        utils.notify('ChaturbateTV', 'Login failed please check your username and password')
...
def Unfollow(id):
    ...
    if '"following": false' in response:
        utils.notify('ChaturbateTV', 'NOT FOLLOWING [COLOR hotpink]{}[/COLOR]'.format(id))
        utils.refresh()
...
def Follow(id):
    ...
    if '"following": true' in response:
        utils.notify('ChaturbateTV', 'FOLLOWING [COLOR hotpink]{}[/COLOR]'.format(id))
        utils.refresh()
...
def Record(id):
    ...
    import xbmc
    search_sites = [
        ('Cloudbate', 'https://www.cloudbate.com/search/{0}/'),
        ('iXXX', 'https://www.ixxx.com/search/{0}/'),
    ]
    names = [s[0] for s in search_sites]
    selection = xbmcgui.Dialog().select('Search recordings on', names)
    if selection != -1:
        url = search_sites[selection][1].format(id)
        try:
            xbmc.executebuiltin('StartAndroidActivity(,android.intent.action.VIEW,,%s)' % urllib_parse.quote(url, safe=''))
        except Exception:
            xbmcgui.Dialog().ok('Open in browser', url)
    utils.eod()
